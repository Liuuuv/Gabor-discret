...
from dual_frame import compute_dual_window, construct_operator_matrix, plot_window
...
def build_by_hand_chi(d_window, k, n):
...
def get_relative_support_tolerance(func: np.ndarray, ratio: float):
    return np.max(np.abs(func)) * ratio

def plot_ratio_support(func: np.ndarray, ax, label: str= "", color='blue', legend='', custom_y_lim_max=0.0, custom_y_lim_min=0.0):
    ratios = np.linspace(1e-5, 1e-2, 600)
    supports = [get_window_support(func, get_relative_support_tolerance(func, ratio)) for ratio in ratios]
    ax.plot(ratios, supports, label=legend, color=color)
    ax.set_title(label)
    ax.set_xlabel("Ratio")
    ...
    ax.margins(x=0.0001, y=None, tight=True)
    if custom_y_lim_min and custom_y_lim_max:
        ax.set_ylim(custom_y_lim_min, custom_y_lim_max)
    start, end = ax.get_xlim()
    ax.xaxis.set_ticks(np.arange(start, end, 5*1e-4))
...
if __name__ == "__main__":
    ...
    relative_support_tolerance = get_relative_support_tolerance(canonical_dual_window, 1e-3)
    plot_window(canonical_dual_window, axes[0], label=f"Duale canonique, support {get_window_support(canonical_dual_window, relative_support_tolerance)}")
    plot_window(d_test_window, axes[1], label="Fenêtre à approcher")
    # axes[0].plot(np.linspace(-0.5,0.5, L),np.abs(d_window)) ### wtf
    
    # plot_window(d_test_window + d_dual_window, axes[4], label="sommé")
    # reconstructed_kperp = approximate_window_from_dual_dir(d_test_window, ax_to_plot=axes[4], ax_phase=axes[5], fig=fig)
    reconstructed_kperp = approximate_window_from_dual_dir(d_test_window)
    plot_window(reconstructed_kperp, axes[2], label=f"Approchée dans K^\perp")
    
    relative_support_tolerance = get_relative_support_tolerance(reconstructed_kperp + canonical_dual_window, 1e-3)
    plot_window(reconstructed_kperp + canonical_dual_window, axes[3], label=f"Duale trouvée, support {get_window_support(reconstructed_kperp + canonical_dual_window, relative_support_tolerance)}")
    
    custom_y_lim_max = np.min(max(np.max(canonical_dual_window), np.max(reconstructed_kperp + canonical_dual_window)))
    custom_y_lim_min = np.max(min(np.min(canonical_dual_window), np.min(reconstructed_kperp + canonical_dual_window)))
    # plot_ratio_support(canonical_dual_window, ax=axes[4], color='blue', legend="Duale canonique")
    plot_ratio_support(canonical_dual_window, ax=axes[4], color='blue')
    plot_ratio_support(reconstructed_kperp + canonical_dual_window, ax=axes[4], color='firebrick', legend="σ=0.05")
    
    exp_part = discretize_window(window=lambda t: (1 - np.exp(-(t/0.06)**2)))
    d_test_window = -canonical_dual_window * exp_part
    reconstructed_kperp = approximate_window_from_dual_dir(d_test_window)
    plot_ratio_support(reconstructed_kperp + canonical_dual_window, ax=axes[4], color='red', legend="σ=0.06")
    
    exp_part = discretize_window(window=lambda t: (1 - np.exp(-(t/0.07)**2)))
    d_test_window = -canonical_dual_window * exp_part
    reconstructed_kperp = approximate_window_from_dual_dir(d_test_window)
    plot_ratio_support(reconstructed_kperp + canonical_dual_window, ax=axes[4], color='orangered', legend="σ=0.07")
    
    exp_part = discretize_window(window=lambda t: (1 - np.exp(-(t/0.08)**2)))
    d_test_window = -canonical_dual_window * exp_part
    reconstructed_kperp = approximate_window_from_dual_dir(d_test_window)
    plot_ratio_support(reconstructed_kperp + canonical_dual_window, ax=axes[4], color='orange', legend="σ=0.08")
    ...
    # plot_window((reconstructed - d_test_window), axes[4], label=f"Différence")
    ...
    # reconstructed_k = approximate_window_from_k(d_test_window, ax_to_plot=axes[6], fig=fig, ax_phase=axes[5])
    ...
    # plot_window((reconstructed_k - d_test_window), axes[8], label=f"Différence")
